...
fn perform_search() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..21 {
        ...
        let parse_playlist = || {
            ...
            let playlist_id = &res["contents"]["twoColumnSearchResultsRenderer"]["primaryContents"]["sectionListRenderer"]["contents"][0]["itemSectionRenderer"]["contents"][i]["playlistRenderer"]["playlistId"];
        ...
        let parse_shelf = || {
            ...
            loop {
                if shelf_renderer["content"]["verticalListRenderer"]["items"][i].is_null() {
                    ...
                    let video_title = &shelf_renderer["content"]["verticalListRenderer"]["items"][i]["videoRenderer"]["title"]["runs"][0]["text"];
                    let video_id = &shelf_renderer["content"]["verticalListRenderer"]["items"][i]["videoRenderer"]["videoId"];
                    let video_owner = &shelf_renderer["content"]["verticalListRenderer"]["items"][i]["videoRenderer"]["ownerText"]["runs"][0]["text"];
        ...
        match channel_renderer {
            Value::Object(_) => { vec.push(parse_channel()); }
            Value::Null => {
                match video_renderer {
                    Value::Object(_) => { vec.push(parse_video()) }
                    Value::Null => {
                        match playlist_renderer {
                            Value::Object(_) => { vec.push(parse_playlist()) }
                            Value::Null => {
                                match shelf_renderer {
                                    Value::Null => { parse_none(); }
                                    Value::Object(_) => { vec.push(parse_shelf()); }
                                    _ => { parse_misread(); }
                                }
                            }
                            _ => { parse_misread(); }
